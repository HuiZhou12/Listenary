...
- 抽出本地与在线共用的歌词时间轴规则，统一普通 LRC、增强 LRC、逐词歌词和 TTML 的行起点、行结束和切换边界
- 逐词歌词按首词时间渲染，下一行默认提前 320ms 接入；上一行只有一个词时保留其实际结束时间，避免过早跳行
- TTML 支持重叠演唱、并行声部和背景人声的活跃行计算
- 长前奏、间奏、短空白行和元数据行按可渲染行规则处理，减少歌词列表跳空或聚焦到空行

**在线歌词解析与同步**

- 在线歌词与本地歌词统一播放页的行切换、聚焦定位和高亮截止处理
- 远程播放时间线采样间隔从 1 秒缩短到 50ms，行切换、暂停、seek 和播放状态变化响应更及时
- 远程逐词高亮改用在线播放时间，不再读取本地 BASS 播放位置
- 修复少于 5 个逐字时间标签的短增强 LRC 被当作普通 LRC 的问题
- 修复在线歌词 offset 跨越零点时的时间戳处理，并清理空的逐字片段

**播放页歌词体验**

- 在线歌词同步本地歌词的字体、字号、字重、颜色、翻译/罗马音排布、模糊、发光、透明度和行间距
- 在线歌词同步本地的边缘渐隐、中心/上方聚焦布局、平滑/弹簧滚动、stagger 入场、拖动后恢复和并行行显示
- 在线歌词支持背景人声占位、并行组高度冻结、聚焦行高亮截止和逐行点击 seek
- 大屏、小屏和沉浸模式沿用各自的本地歌词对齐参数，避免不同播放页聚焦位置不一致
- 恢复在线播放页面的“歌词设置”入口，字号、粗细、对齐、模糊、翻译和罗马音选项与本地歌词共用
